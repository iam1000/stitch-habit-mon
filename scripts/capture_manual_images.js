...
// 저장 경로 설정 (src/md/images)
const SAVE_DIR = path.join(__dirname, '../src/md/images');
...
async function run() {
    ...
    try {
        ...
        // - case A: 로그인 화면 (input[type="email"])
        // - case B: 메인 화면 (로그아웃 버튼 등 식별자, 여기서는 헤더나 네비게이션)

        console.log('⏳ 로그인 상태 식별 중...');

        try {
            // 두 가지 요소를 동시에 기다림 (먼저 뜨는 쪽이 승자)
            const result = await Promise.race([
                page.waitForSelector('input[type="email"]', { timeout: 5000 }).then(() => 'LOGIN_NEEDED'),
                // 메인 화면 요소: 네비게이션 바(nav) 또는 로그아웃 버튼
            ...
            if (result === 'LOGIN_NEEDED') {
                ...
                // 로그인이 완료되어 메인 화면 요소(nav)가 뜰 때까지 대기
